...
int hasWon(int (*boards)[COLS][ROWS], int board_num) {
  ...
  for (int i=0; i<ROWS; i++){
    right_diag_sum += boards[board_num][ROWS-1-i][i];
  }
...
int oneLeft(int *boards, int num_boards, int *winning_board) {
  // Check if there is one board left that hasn't won yet
  //
  // <boards> is an array of ints where each element is either a board number
  //  if the board is still in play or 0 if it is not
...
int play_games(char *input, int (*boards)[COLS][ROWS], int num_boards){
  ...
  // Create an int to track boards
  int winning_boards[num_boards];
  for (int i=0; i<num_boards; i++) {
    winning_boards[i] = i;
  }
  ...
  do {
    ...
    for (int board=0; board<num_boards; board++) {
      if (winning_boards[board] != 0 && hasWon(boards_copy, board)) {
        winning_boards[board] = 0;

        int winning_board = 0;
        if (oneLeft(winning_boards, num_boards, &winning_board)) {
          // found the last board to win
          while (!hasWon(boards_copy, winning_board)) {
            // play it until it finally wins
            tok = strtok(NULL, ",");
            num = atoi(tok);
            step(boards_copy, num_boards, num);
          }
          printf("board %d is last to win when num %d is drawn\n", winning_board, num);
          printf("\nUnmarked board:\n---\n");
          print_board(boards, winning_board);
          printf("\nMarked board:\n---\n");
          print_board(boards_copy, winning_board);
          int sum = get_board_sum(boards_copy[winning_board]);
          return sum * num;
        }
      }
    }
